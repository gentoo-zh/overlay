...
inherit kernel-install toolchain-funcs unpacker verify-sig cjktty
...
SRC_URI+="
	...
	amd64? (
		cjk32? (
			https://distfiles.gentoozh.org/gentoo-cjk-kernel/amd64/$(ver_cut 1-2)/${BINPKG}.amd64.cjk32.gpkg.tar
		)
		!cjk32? (
			https://distfiles.gentoozh.org/gentoo-cjk-kernel/amd64/$(ver_cut 1-2)/${BINPKG}.amd64.gpkg.tar
		)
	)
"
S=${WORKDIR}
...
KV_LOCALVERSION='-gentoo-cjk-dist-bin'
...
src_prepare() {
	...
	# The published binary carries these patches, so modules_prepare has to
	# see the same tree or an external module gets the wrong Module.symvers.
...
src_configure() {
	...
	# The published package is built with USE=-modules-sign, which leaves the
	# file empty, so an empty cert must not override the user's own.
	if [[ -s ${kernel_dir}/certs/signing_key.x509 ]]; then
		openssl x509 \
