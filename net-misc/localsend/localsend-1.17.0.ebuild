...
RUST_MIN_VER="1.80.0"
# pinned by FLUTTER_VERSION in .github/workflows/linux_build.yml; re-check it
# on every bump
FLUTTER_PV="3.24.5"

inherit desktop flutter-app multiprocessing rust xdg

DESCRIPTION="An open-source cross-platform alternative to AirDrop"
HOMEPAGE="https://localsend.org"

# Regenerate with pubspec2ebuild.py on the pubspec.lock left by
# scripts/remove_proprietary_dependencies.sh plus `flutter pub get`.
PUB_HOSTED=(
...
LICENSE+=" BSD BSD-2 LGPL-3 MIT MPL-2.0"
# vendored rhttp crates
LICENSE+="
...
# the version of the rhttp package in PUB_HOSTED, whose bundled cargokit copy
# has to be told to stay offline
RHTTP_PV="0.10.0"
...
src_prepare() {
	# drops the proprietary in-app-purchase code; the script cd's into app/ itself
	...
	# still lists the removed proprietary deps, so let pub re-resolve offline
	rm "${S}"/pubspec.lock || die

	# cargokit drives the toolchain through rustup; shim it onto the system Rust
	mkdir -p "${T}"/bin || die
	cp "${FILESDIR}"/rustup "${T}"/bin/rustup || die
	...
	# cargokit resolves its own build tool at build time, and reaches the network
	# doing so unless the copy in the cache is told not to
	sed -i -e 's/pub get --no-precompile/& --offline/' \
		"${EPUB_CACHE}"/hosted/pub.dev/rhttp-${RHTTP_PV}/cargokit/run_build_tool.sh || die
...
src_compile() {
	local -x CARGO_HOME="${T}/cargo-home"
	local -x CARGO_NET_OFFLINE=true
	local -x CARGO_BUILD_JOBS="$(makeopts_jobs)"
